pub fn test_rust_iterator(){
    let mut fruit_list = vec!["Strawberry", "Avocado", "Carrot", "Bananas", "Apple"];

    let nut_list = vec!["Walnut", "Peanuts", "Almonds", "Groundnuts"];

    fruit_list.push("Mango");
    ...
    // `let full_names = first_name_strings.zip(last_name_strings);` is creating a new iterator called
    // `full_names` by zipping together the iterators `first_name_strings` and `last_name_strings`.
    let full_names = first_name_strings.zip(last_name_strings);

    full_names.clone().for_each(|name| println!("{} {}", name.0, name.1));

    // This code snippet is iterating over the `full_names` iterator using the `enumerate()` method,
    // which adds an index to each element in the iterator.
    for (index, name) in full_names.enumerate() {
        println!("Index: {} First Name: {}, Second Name: {}", index, name.0, name.1);
    }
